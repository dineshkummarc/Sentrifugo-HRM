...
IF(FIND_IN_SET('enabled',GROUP_CONCAT(sun_project_status,',',mon_project_status,',',tue_project_status,',',wed_project_status,',',thu_project_status,',',fri_project_status,',',sat_project_status))!=0,'Enabled','No Entry'))))))";
    ...
    public function getEmployeesForMonthly($manager_id = "",$year,$month,$search="",$clicked_status,$emp_list_flag="",$week="",$current_page)
    {
        ...
        $billableHours = "if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.week_duration as time)))), '%H:%i'), '00:00')";
        $unbillableHours = "if(tpt.is_billable is null or tpt.is_billable = 0, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.week_duration as time)))), '%H:%i'), '00:00')";

        $time_status = " GROUP_CONCAT(DISTINCT week_status) ";

        $selectEmpTimesheetsQuery = "SELECT ms.user_id AS empid, ms.userfullname AS empname,
									".$duration." AS duration, 
									
									" . $billableHours. " AS billable_hours, 
									" . $unbillableHours. " AS unbillable_hours,
        ...
        $selectEmpTimesheetsQuery.=" LEFT JOIN tm_project_tasks AS tpt ON et.project_task_id = tpt.id ";
        ...
        $selectEmpTimesheetsQuery.=$where;
        $selectEmpTimesheetsQuery.=" GROUP BY ms.user_id, tpt.is_billable";

        if($clicked_status != "all"){
            $selectEmpTimesheetsQuery.=" HAVING time_status=\"".$stat_arr[$clicked_status]."\" ";
        }

        $finalQuery = "select empid, empname,
                      IF(FIND_IN_SET('blocked', max(time_status)) != 0, 'Blocked', 
                       IF(FIND_IN_SET('submitted', max(time_status)) != 0, 'For Approval', 
                       IF(FIND_IN_SET('rejected', max(time_status)) != 0, 'Rejected', 
                       IF(FIND_IN_SET('approved', max(time_status)) != 0, 'Approved', 
                       IF(FIND_IN_SET('saved', max(time_status)) != 0, 'Saved', 
                       IF(FIND_IN_SET('enabled', max(time_status)) != 0, 'Enabled', 'No Entry')))))) AS time_status, 
                        max(ts_week) as ts_week,
                        max(ts_year) as ts_year,
                        max(ts_month) as ts_month,
                        if(max(billable_hours) is not null, max(billable_hours), '00:00') as billable_duration,
                        if(max(unbillable_hours) is not null,max(unbillable_hours), '00:00' ) as unbillable_duration, 
                        if(SEC_TO_TIME(TIME_TO_SEC(cast(max(billable_hours) as time)) + TIME_TO_SEC(cast(max(unbillable_hours) as time))) is null, '00:00', time_format(SEC_TO_TIME(TIME_TO_SEC(cast(max(billable_hours) as time)) + TIME_TO_SEC(cast(max(unbillable_hours) as time))), '%H:%i')) as duration
                        from ( " . $selectEmpTimesheetsQuery .  " ) result group by empid";


        //$finalQuery .= " LIMIT ".$current_index.",".$per_page;
        ...
        while ($row=$res->fetch())
        {
            ...
            $fin_arr[$i]['emp_name']=$row['empname'];
            $fin_arr[$i]['duration']=$row['duration'];
            $fin_arr[$i]['billable_duration']=$row['billable_duration'];
            $fin_arr[$i]['unbillable_duration']=$row['unbillable_duration'];
            $fin_arr[$i]['time_status']=$row['time_status'];
            $fin_arr[$i]['ts_week']=$row['ts_week'];
            $fin_arr[$i]['ts_year']=$row['ts_year'];
            $fin_arr[$i]['ts_month']=$row['ts_month'];
...
//        }
    ...
    public function getWeeklyTimesheetData($empId,$year,$month,$week,$project_ids="") {
        $select = $this->select()->distinct()
            ...
            ->from(array('pte'=>'tm_project_task_employees'),array(
                'pte.project_id','p.project_name','pte.project_task_id','t.task','pt.is_billable','sun_duration'=>'ifnull(et.sun_duration,0)','mon_duration'=>'ifnull(et.mon_duration,0)',
        ...
        $select->where("et.is_active=1 and pte.emp_id =".$empId);
        if($project_ids != ""){
    ...
    public function getProjectWiseWeeklyAggregatedData($ts_year = 2020, $ts_month = 12)
    {
        $db = Zend_Db_Table::getDefaultAdapter();
        $select = "SELECT
                           project_id,
                        ...
                        from
                           (
                              SELECT
                                 ...
                                 et.sat_date,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.sun_duration as time)))), '%H:%i'), '00:00') AS sun_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.sun_duration as time)))), '%H:%i'), '00:00') AS sun_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.mon_duration as time)))), '%H:%i'), '00:00') AS mon_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.mon_duration as time)))), '%H:%i'), '00:00') AS mon_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.tue_duration as time)))), '%H:%i'), '00:00') AS tue_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.tue_duration as time)))), '%H:%i'), '00:00') AS tue_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.wed_duration as time)))), '%H:%i'), '00:00') AS wed_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.wed_duration as time)))), '%H:%i'), '00:00') AS wed_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.thu_duration as time)))), '%H:%i'), '00:00') AS thu_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.thu_duration as time)))), '%H:%i'), '00:00') AS thu_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.fri_duration as time)))), '%H:%i'), '00:00') AS fri_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.fri_duration as time)))), '%H:%i'), '00:00') AS fri_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.sat_duration as time)))), '%H:%i'), '00:00') AS sat_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.sat_duration as time)))), '%H:%i'), '00:00') AS sat_duration_unbillable,
                                 if(tpt.is_billable = 1, time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.week_duration as time)))), '%H:%i'), '00:00') AS week_duration_billable,
                                 if(tpt.is_billable is null 
                                 or tpt.is_billable = 0 , time_format(SEC_TO_TIME(sum(TIME_TO_SEC(cast(et.week_duration as time)))), '%H:%i'), '00:00') AS week_duration_unbillable 
                              FROM
                                 `tm_projects` AS `project` 
                                 JOIN
                                    `tm_emp_timesheets` AS `et` 
                                    On project.id = et.project_id 
                                 LEFT JOIN
                                    `tm_project_tasks` AS `tpt` 
                                    ON et.project_task_id = tpt.id 
                                 INNER JOIN
                                    `tm_ts_status` AS `ts` 
                                    ON ts.emp_id = et.emp_id 
                                    and et.ts_year = ts.ts_year 
                                    and et.ts_month = ts.ts_month 
                                    and et.ts_week = ts.ts_week 
                                    and 
                                    (
                                       et.project_id = ts.project_id 
                                       OR ts.project_id IS NULL 
                                    )
                                 LEFT JOIN
                                    `tm_projects` AS `p` 
                                    ON p.id = ts.project_id 
                                    and p.id = et.project_id 
                              WHERE
                                 (
                                    et.ts_year = $ts_year 
                                    and et.ts_month = $ts_month 
                                 )
                              GROUP BY
                                 project.id,
                                 `ts`.`ts_week`,
        ...
        foreach ($response as $item){
            if(!isset($weekWiseData[$item['ts_week']]))
                $weekWiseData[$item['ts_week']] = [];

            $weekWiseData[$item['ts_week']][] = $item;
        }
